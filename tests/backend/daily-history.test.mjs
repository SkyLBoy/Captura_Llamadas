...
try {
  ...
  async function insert(agent, date, state = 'closed') {
    ...
    const assignment = (await engine.query('INSERT INTO contact_assignments(client_id,campaign_id,agent_id) VALUES($1,$2,$3) ON CONFLICT DO NOTHING RETURNING assignment_id', [client, campaign, agent])).rows[0]
